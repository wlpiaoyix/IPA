...
- (sqlite3_int64) executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    while (sqlite3_step(pStmt)==SQLITE_ROW){
        _id = sqlite3_column_int64(pStmt, 0);
    }
    ...
    _isExecutingStatement = NO;
    return (rc == SQLITE_DONE || rc == SQLITE_OK)?(_id ? _id : -1):0;
}
...
- (sqlite3_int64)executeUpdate:(NSString*)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeUpdate:sql error:nil withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (sqlite3_int64)executeUpdate:(NSString*)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeUpdate:sql error:nil withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (sqlite3_int64)executeUpdate:(NSString*)sql withVAList:(va_list)args {
